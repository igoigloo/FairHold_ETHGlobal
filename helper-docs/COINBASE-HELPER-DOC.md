...
- **4.1% USDC yield** automatically earned on balances
...
### Key Features
- **Embedded wallets** for clients and vendors with 4.1% USDC rewards
...
- **Cross-network support** (Base, Ethereum, Polygon, etc.)
...
### Yield Strategy Clarification

```typescript
interface YieldEarningStrategy {
  embeddedWallets: {
    clients: "4.1% USDC yield on wallet balances"
    vendors: "4.1% USDC yield on received payments"
    automatic: "Built-in yield generation"
  }
  
  serverWallets: {
    escrow: "No yield - temporary holding only"
    purpose: "Fast automated transactions"
    duration: "Minimal hold time before transfer"
  }
  
  strategy: "Minimize time in server wallet, maximize time in embedded wallets for yield"
}
```
...
export class JointWalletService {
  ...
  /**
   * Calculate yield earned in vendor's embedded wallet
   */
  async calculateVendorEmbeddedWalletYield(vendorWalletAddress: string): Promise<number> {
    try {
      const yieldInfo = await this.cdp.embedded.getYieldInfo({
